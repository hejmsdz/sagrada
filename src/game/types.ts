...
export class Board {
  ...
  constructor(board: OptionalDice[][]) {
    const isValid = board.length === NUM_ROWS && board.every(row => row.length === NUM_COLUMNS);
    if (!isValid) {
  ...
  forEach(callback: (dice: OptionalDice, rowIndex: number, columnIndex: number) => void): void {
    this.board.forEach((row, rowIndex) => row.forEach((dice, columnIndex) => callback(dice, rowIndex, columnIndex)));
  }

  createMask(predicate: (dice: OptionalDice, rowIndex: number, columnIndex: number) => boolean): Mask {
    return this.board.map((row, rowIndex) => row.map((dice, columnIndex) => predicate(dice, rowIndex, columnIndex)));
  }
